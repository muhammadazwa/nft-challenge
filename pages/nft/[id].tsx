...
function NFTDropPage({collection}: Props) {
  const [claimedSuppy, setClaimedSuppy] = useState<number>(0);
  ...
  const mintNft = () => {
    ...
    nftDrop.claimTo(address, quantity).then( async (tx) => {
       ...
       toast('HOORAY you\'ve succesfully minted!!!', {
            duration: 8000,
            style: {
                background: 'green',
                color: 'white',
    ...
    <div className='flex h-screen flex-col lg:grid lg:grid-cols-10'>
        ...
        <div className='flex flex-1 flex-col p-12 lg:col-span-6'>
            ...
            <header className='flex items-center justify-between'>
                ...
                <button onClick={() => {address ? disconnect() : connectWithMetamask()}} className='rounded-full bg-rose-400 px-4 py-2 text-xs font-bold 
            ...
            { address && 
            <p className='text-center text-sm text-rose-400'>You're logged in with wallet 
            ...
            {/* button */}
            <button onClick={mintNft} disabled={loading || claimedSuppy === totalSupply?.toNumber() || !address} 
